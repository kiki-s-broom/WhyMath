...
### 4회차 — 동시간대 A/B (2026-09-18 · `5ef72151` · **같은 [peak] 구간**)

3회차의 시간대 교란을 없애려고 같은 창에서 공급사만 바꿔 다시 쟀다
(`$env:WHYMATH_OPENROUTER_ALLOWED_PROVIDERS = '["deepinfra"]'` — 그 창에서만 유효).

| (같은 peak 구간) | **deepinfra** | baseten |
|---|---|---|
| 실패 | **0/20** | 8/20 |
| 재시도 | **0회** | 28회 / 17건 |
| 집계 제외 | **0건** | 8건 |
| 지연 p50 | 36,939ms | **5,348ms** |
| 지연 p95 | 212,563ms | **23,151ms** |
| 토큰(회당) | 465 / 1,935 | 468 / 1,779 |

#### 3회차 결론의 정정 두 개

**① "429는 공급사의 성질이 아니다"는 과했다.** *메커니즘*은 같다(공유 풀 ·
`is_byok=false`). 하지만 **그 풀 안의 용량은 공급사마다 크게 다르다** — 같은 시간대에
deepinfra 0건, baseten 8건이다. 공급사 선택이 가용성에 영향을 **준다.**

**② 시간대 가설도 틀렸다.** deepinfra 3회를 나란히 놓으면:
...
가장 붐빌 것이라던 peak에서 가장 깨끗했다. 시간대로 설명되지 않는다.

#### 이 표본으로는 공급사를 고를 수 없다

같은 공급사·같은 시험지로 **30% → 15% → 0%**가 나왔다. 이 분산에서 20회 한 회차로
공급사를 고르는 것은 노이즈를 읽는 것이다. **이 세션은 그 노이즈로 판단을 세 번 뒤집었다**
(deepinfra 유지 → baseten 교체 → 구조 문제 → 재정정).

표본을 키울 수도 있지만 그 전에 짚을 것이 있다 — **우리는 실제로 돌릴 구성이 아닌 것을
재고 있다.** BYOK을 붙이면 공유 풀을 쓰지 않으므로 지금 재는 대기열 통계는 채택 후 상황을
전혀 대표하지 않는다. 회차를 더 태워도 버릴 숫자다.

#### 출력 토큰 미스터리 해소 — 가설 ⓒ 기각

회당 출력: 634 → 1,518 → **1,935**(deepinfra) · 1,779(baseten). 공식 API 1,985와 같은
대역이다. **1회차의 634가 이상값이었고, OpenRouter가 추론 토큰을 usage에서 빠뜨린다는
증거는 없다.** 청구액 대조 없이도 이 축은 닫힌다.

비용도 이 토큰으로 확정된다(OpenRouter 단가 · `claude-sonnet-4-6` $0.004767 대비):

| | 회당 | 우위 |
|---|---|---|
| deepinfra ($0.20/$0.60) | $0.00125 | **3.8배 저렴** |
| baseten ($0.30/$1.20) | $0.00228 | 2.1배 저렴 |


### 결론 — 기본값은 `deepinfra`, 가용성 축은 BYOK까지 보류 (2026-09-18 · Kiki 판단)

**기본 허용목록을 `deepinfra`로 되돌렸다.** `baseten`으로 옮겼던 근거(OpenRouter 표의
`Uptime 100.00%`)가 무효로 확인됐기 때문이다 — 그 열은 *공급사 엔드포인트의 생존*을 재지
*우리가 공유 풀을 뚫고 거기 도달하는지*를 재지 않는다.

되돌린 이유 셋:
1. 옮긴 근거가 무효였다.
2. **회당 비용이 약 절반**이다($0.00125 vs $0.00228 · Sonnet 대비 3.8배 vs 2.1배).
3. 검증 가능한 세 축(관할·데이터 정책·양자화)은 deepinfra도 처음부터 충족했다.

`baseten`은 **대안으로 표에 남겨 둔다** — 관할·정책·정밀도가 모두 등재돼 있어 갈아 끼울 때
근거를 다시 모을 필요가 없고, 그 사실을 테스트가 계약으로 고정한다
(`test_the_documented_alternative_satisfies_the_same_axes`).

#### 가용성 축을 판정에서 뺀 이유

네 회차의 `429` 실패율이 **30% → 15% → 0%**로 흔들렸고 **마지막(0%)이 peak 구간**이었다.
시간대로 설명되지 않으며, 20회 표본으로는 공급사를 가를 수 없다. 이 세션은 실제로 이
분산을 근거로 기본값을 두 번 뒤집었다 — 그것을 기록으로 남긴다.

실패 원인이 `limit_source=upstream_provider_shared_pool` · `is_byok=false`(공유 풀)이므로
**BYOK을 붙이면 이 축 자체가 사라진다.** 그때까지 이 문서의 가용성 수치는 *우리가 쓰지 않을
구성*을 재는 값이며, 더 큰 표본을 태워도 버릴 숫자다.

#### 남은 사람 작업 — 없다 (BYOK 미채택 · 2026-09-18 Kiki 판단)

이 문서의 앞 버전은 BYOK 등록을 "남은 사람 작업"으로 올렸다. **철회한다.**

Kiki 판단: *"우리는 일단 개발과정에 사용할건데 byok까지 할 거 있나?"* — 맞다. BYOK가 사는
것은 가용성 수치의 깨끗함이고, 그 수치가 필요한 것은 **트래픽이 붙을 때**다. 지금 이 경로를
쓰는 것은 20회 단위 측정(회차당 $0.03 미만)뿐이고, 재시도 배선 이후 마지막 deepinfra 회차는
집계 제외 0건이었다. 반대로 BYOK는 공급사 계정·결제수단을 새로 만들어 **청구서를 2장**으로
늘리고, 자동 폴백 차단을 다시 실측해야 한다.

즉 이 세션이 "측정 완결성"을 목적 자체로 삼아 사람 작업을 만들어 냈다. 개발 단계에
편익이 없는 비용이므로 짚어 둔다.

가용성 축은 **발동 조건을 붙여 승계**한다 (`ARCH-56-openrouter-availability-axis`) —
산문 보류는 CLAUDE.md가 금지하는 만료 없는 유예이므로, 세 조건 중 먼저 오는 것이 착수
시점이 되게 대장에 못박았다: ⓐ 학생 대면 또는 상시 배치 트래픽 투입 결정 ⓑ 기본
허용목록 변경 판정 발생 ⓒ 한 회차 `429` 실패율 50% 초과.
...
| 기준 | 결과 | 근거 |
|---|---|---|
| (a) 라우터·관할 게이트 통과해 실제로 도는가 | **충족** | 4회차 전건 라이브 성공. `Router()` 경유라 좌석 계약 위반 0건 |
| (b) 공급사 고정이 성립하는가 | **충족** | 4회차 전부 불일치 0건 — `only` 1곳 + `allow_fallbacks=false`가 의도대로 동작(deepinfra가 아니면 응답 대신 실패) |
| (c) 회당 비용 | **충족** | $0.00125 vs Sonnet 4.6 $0.004767 → **3.8배 저렴** |
| (d) 지연·가용성 | **보류** | 분산이 커서 20회로 못 가름(30%→15%→0%). 공유 풀 구성은 우리가 배포할 것이 아니다 → `ARCH-56` |
...
| 축 | 처분 | 이유 |
|---|---|---|
| 품질 강등전(Wilson 경계) | **판정 기준 아님** | 오류 검출은 WhyMath 완성 후 **다축 오케스트레이션**이 담당(2026-09-17 Kiki 범위 정정). 단일 모델의 결함 검출률로 개발 도구를 가르지 않는다 |
| 요금 구간(peak/off_peak) 분리 집계 | **무의미** | 피크 2배 단가는 DeepSeek 공식 API만의 개념이고 OpenRouter는 평평하다 |
| DeepSeek 유료 API 학습 약관(구 acceptance ⑤) | **해당 없음** | 채택 경로가 OpenRouter이고 `data_collection="deny"`가 공급사 단위로 집행된다. 공식 API를 쓰지 않으므로 그 약관은 판정 입력이 아니다. `deepseek_allow_internal_corpus` 기본값 `False`는 **그대로 유지**한다(공식 API 경로가 남아 있으므로 방어를 풀 이유가 없다) |

#### 배경 대조로만 인용하는 것

DeepSeek 공식 API arm 실측(2026-09-17 · off_peak 80회)은 폐기하지 않는다. 같은 시험지·같은
하네스로 잰 대조군이라 OpenRouter 수치를 읽을 때 배경이 된다 — 단 **채택 근거가 아니다.**
청구서 대조로 검증된 유일한 축이기도 하다(계정 30일 사용량 101요청 / 236,012토큰 / $0.11이
프로브 1 + 스모크 20 + 배틀 80 = 101콜, 508 + 48,995 + 186,509 = 236,012토큰과 정확히 일치).

#### 이 판정을 내리며 이 세션이 틀렸던 것

1. **저작 경로가 아니라 판정자(judge) 역할로 쟀다.** acceptance ①은 대상이 *코퍼스 문항
   저작·개념 추출*이라고 적혀 있었는데, 그 하네스가 이미 있었기 때문에 결함 주입 강등전을
   쟀다. 그래서 얻은 "4배 비싸다·p95 39초"는 **우리가 쓰지 않을 작업 유형에서의 최악 조건**이다.
2. **공급사 표의 Uptime 열을 해답으로 읽었다.** 그 열은 엔드포인트 생존을 재지 우리가 공유
   풀을 뚫고 도달하는지를 재지 않는다. 오류 본문이 두 번 같은 말(`upstream_provider_shared_pool`)을
   했는데도 공급사 교체를 먼저 권했다.
3. **노이즈로 기본값을 두 번 뒤집었다.** 같은 공급사·같은 시험지에서 30%→15%→0%가 나오는
   분산이었다.
4. **무변별 검증 기준을 냈다.** "`is_byok`가 `true`로 바뀌는지 보라"고 했는데 그 필드는 429
   오류 본문에만 실린다 — BYOK가 작동하면 읽을 기회가 없다.
5. **편익 없는 사람 작업을 만들었다.** 위 철회분.
