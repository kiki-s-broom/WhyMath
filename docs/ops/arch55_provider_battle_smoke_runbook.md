...
<https://openrouter.ai/settings/integrations> 에 공급사 자체 API 키를 등록하면 `is_byok`가
`true`가 되고 공유 풀이 아니라 그 계정 몫의 rate limit을 쓴다. deepinfra 계정이 필요하다
(`baseten`도 `BYOK supported`). 붙인 뒤 같은 20회를 다시 돌리면 (d) 축이 채워지고 ARCH-55의
채택 판정을 쓸 수 있다.
