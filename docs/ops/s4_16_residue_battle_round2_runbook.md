...
리포트가 그 수치를 직접 낸다). 0.05 아래로 내리려면 대조군 `n>=52` 가 필요한데 코퍼스가 34문이다
...
2. **코퍼스를 확장해 0.05 를 겨냥** — 대조군 52문 이상을 만든 뒤 재측정한다(별도 태스크).
...
> **[정정 2026-09-25]** 이 절의 종전 수치 "`n>=51`"·"51문 이상"은 틀렸다 — 대조군 51문 전건
> 무오검출이어도 95% 단측 Wilson 상한은 `0.0504`로 0.05를 넘지 못하고, 52문에서 `0.0495`가 된다
> (`harness/wilson.py` 실측). 위 두 곳을 52로 고쳤다. Kiki는 2026-09-25에 **3안**을 골랐고 그 판정은
> 이 수치에 의존하지 않는다. 판정 결과와 후속 처분은 이력 문서 §5·§6이 정본이다.
