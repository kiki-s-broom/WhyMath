...
## 3a. 기간은 목표이지 진도 제한이 아니다 (2026-09-24 Kiki 지정 · 기본방침)

> Kiki 원문: "기간을 세팅한 것은 적절한 목표를 설정한 것이지 진도를 제한하기 위한 것이 아냐" ·
> "기간은 단축하는 것이 이로운 것이고, 내용은 채우는 것이 유리한 것이다"

계획서·지시문·게이트에 적힌 **날짜·주차·Phase 기간은 목표(도달 시한)이다. 착수 하한이 아니다.**
앞 단계의 *내용*이 준비됐으면 달력이 오기 전에 착수하고, 판정 조건이 채워졌으면 날짜 전에 판정한다.

| 축 | 방침 | 하네스에서 뜻하는 것 |
|---|---|---|
| 기간 | **줄일수록 이롭다** | 날짜는 "늦어도 이때까지"다. "이때 전에는 하지 마라"로 읽지 않는다 |
| 내용 | **채울수록 유리하다** | 기간이 줄어도 요건(acceptance·Release Gate 요건)은 줄이지 않는다. 시한에 요건이 모자라면 범위를 깎아 PASS로 만들지 않고 FAIL로 판정한다 |
| 순서 | **내용 선행만 순서를 강제한다** | `depends_on`은 *앞 산출물이 있어야 뒤 작업이 성립하는* 관계에만 건다. 주차 경계·달력을 흉내 내려고 선행을 걸지 않는다 |
| 판정 게이트 | **조건 충족 시 조기 판정 허용** | 게이트 제목의 날짜(예: `G-p3-g1-week1`의 "(11/1)")는 **판정 목표일**이다. 판정 조건이 채워졌으면 그날을 기다리지 않고 `gates clear`한다 |

**이 방침이 바꾸지 않는 것**:
- **범위(스코프) 결정** — Not Now 12종처럼 "v1.0 이후"로 정한 것은 기간이 아니라 *무엇을 할지*의 결정이다. 문구에 "11월 착수 금지"가 섞여 있어도 그 실질은 스코프이며, 해제 조건은 달력이 아니라 게이트 `G-not-now-v1-release-recheck` 판정이다.
- **조건 트리거** — "출시 직전에 착수"(`S3-01`)처럼 날짜가 아니라 *상태*에 걸린 착수 조건.
- **사람 게이트** — 판정의 주체는 그대로 Kiki다. 바뀌는 것은 *언제* 판정할 수 있는가뿐이다.
- **단계(stage) 정렬** — stage는 정렬 1차 키일 뿐 후보를 거르지 않으므로, 이미 이 방침과 맞다.

**집행 지점**:
- 코드 — selector는 날짜로 후보를 거르지 않는다. 이 성질은 `tests/harness/test_schedule_is_target_not_limit.py`가 AST 검사로 동결한다. selector가 `datetime`·`time`을 임포트하거나 `today()`·`now()`를 호출하면 red이며, 검출기의 변별력은 같은 테스트가 주입으로 확인한다.
- 대장 — 주차 경계 대용으로 걸었던 선행(`EOS-50` → `P3-10`)을 뗐다. 주차 판정 게이트가 붙은 `P3-04`·`P3-08`은 게이트를 유지하되, 위 "조기 판정 허용"에 따라 그 게이트는 날짜가 아니라 판정 조건으로 풀린다. 처분 기록 = `docs/strategy/phase3_math_eos_completion_backlog_crosswalk.md` §22-9.
- **한계(명시)** — 계획서·게이트 *문면*에 남은 날짜는 고치지 않았다(게이트 정정 CLI 부재 · `HARN-124` 소유). 문면의 날짜는 이 절에 따라 목표일로 읽는다.
...
### 3b-5. git 원복 계열 가드 — 미커밋 작업분 삼킴 차단 (HARN-136)

`git checkout -- <경로>` · `git restore <경로>` · `git stash` 는 아직 커밋되지 않은
작업분을 **무증상으로** 지운다(에러도 경고도 없고 `git status` 에서 파일이 조용히
사라질 뿐이다). 이 저장소에서 두 번 났다 — 2026-08-10 `OPS-24`(미커밋 구현분
+59/-6 소실) · 2026-09-22 `EOS-128`(이벤트 약 10건 소실 · 감사 추적 단절).

**집행 지점**: PreToolUse(Bash) 훅 `.claude/hooks/git_revert_guard.py`
(`.claude/settings.json` 등록 · 배선 동결 `tests/infra/test_git_revert_guard.py`).

**판정 기준은 "git 원복 명령인가"가 아니라 "그 경로에 잃을 것이 있는가"다.**
전건 차단하면 정상 브랜치 전환까지 막혀 사람이 게이트를 끈다. 대상 경로에 실제
미커밋 변경이 있을 때만 exit 2로 막고, 깨끗한 트리에서는 한 번도 발화하지 않는다.

| 상황 | 동작 |
|---|---|
| 대상 경로에 미커밋 변경 있음 | **차단** + 변경 목록 + 대안(뮤테이션 원복은 `cp` 백업 경유) |
| 대상 경로가 깨끗함 (다른 경로가 더러워도) | 통과 — 판정은 경로 단위다 |
| `checkout -b`·`-B`·`stash pop|list|show` | 통과 — 경로를 되돌리지 않거나 오히려 되살린다 |
| `restore --staged` 단독 | 통과 — 인덱스만 되돌리므로 작업 트리 손실이 아니다 |
| `stash drop`·`clear` | 스태시가 비어 있지 않으면 차단 |
| 명령 파싱 실패 · `git status` 조회 실패 | **차단** — 모른다를 '없다'로 접지 않는다 |
| 탈출구 `#버려도됨` / `#discard-ok` | 통과하되 `.claude/logs/git_revert_blocks.jsonl` 에 기록 |

**한계(막지 못하는 것을 막는다고 말하지 않는다)**: 이 훅은 **Bash 도구를 경유하는
명령만** 본다 — 스크립트 안에서 호출되는 git, 다른 도구가 부르는 git, 사람이 자기
터미널에서 치는 git 에는 닿지 않는다. 탐지는 명령 문자열 파싱이므로 변수·`eval`·
