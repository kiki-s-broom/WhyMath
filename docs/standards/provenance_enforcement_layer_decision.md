...
**복원 명령**: `python -m whymath_backend.l1.problem_bank.populate --all`(저장소 루트에서).
전 문제은행 코퍼스(`data/corpus/problem_bank_*/problems.jsonl`)를 순차 적재한다. 실측
1분 31초로 14,034건 전건 복원, 즉시 재실행 시 신규 0건·행 수 불변(멱등). `--problems`와는
상호 배타이며, 스캔 0건이면 **exit 2**로 실패한다 — 잘못된 디렉터리에서 돌린 회차가
"성공적으로 아무것도 안 함"으로 보고되면 운영자가 복원됐다고 믿는다.

**전제 1건(명시)**: 복원은 *코퍼스에서 적재된* 문항에만 해당한다 — 관리자 REST로 만들어진
문항은 코퍼스에 원본이 없어 이 경로로 복원되지 않는다(`LIC-09` 소유).
