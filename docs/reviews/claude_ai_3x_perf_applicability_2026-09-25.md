...
> **성격**: 조사 문서. 코드 변경 0건. §6의 태스크 후보 9건을 **전부 등재**했다(2026-09-25 Kiki 지정 두 차례 — 최소 묶음 3건 `OPS-95`·`MOB-24`·`OPS-96`, 이어서 나머지 6건 `OPS-97`·`OPS-98`·`PED-38`·`PED-39`·`REC-12`·`MOB-25`). 착수 순서는 대장(`depends_on`·EOS 등급)이 정한다(§2-E: 의사결정 우선순위상 UX는 5순위이고, 지금은 12/31 내부 완성 집중 기간이다).
...
등재는 `backlog.py add`로만 한다. 번호는 CLI가 배정한다(「태스크 ID 번호를 추론으로 배정 금지」). 9건 모두 CLI가 제안한 번호를 그대로 썼다. 선행 관계는 산문이 아니라 대장의 `depends_on`으로 걸었다.

| 가칭 | 권고 | 레이어 | 크기 | 선행·연결 | 등재 |
|---|---|---|---|---|---|
| 경로별 서버 지연 계측 + Server-Timing | R1 | backend | 소 | `OPS-30`(범위 밖으로 넘긴 S5를 인수) | ✅ `OPS-95` (P1) |
| 핵심 흐름 결정론 예산 게이트(ratchet) | R2 | backend·infra | 중 | R1(상관 검증에 경로별 지연이 필요), `EOS-114`(next-problem 쿼리 실측과 합류 가능) | ✅ `OPS-97` (P2 · 선행 `OPS-95`) |
| 코치 턴 도구 호출·지연 분포 실측(단순 턴/풀이 턴 분리) + 예산 정합 | R3 ①③④ | backend·L4 | 중 | `OPS-75`(재측정) 선행, 판정은 Kiki | ✅ `OPS-98` (P1 · 선행 `SEC-34`) |
| WH-1 fast path(§5.3) 배선 — 기본 OFF, 애매하면 풀 루프 | R3 ② | backend·L4 | 중 | 위 실측 결과(단순 턴 비용이 유의할 때만). 설계는 있고 소유자가 없다 | ✅ `PED-38` (P1 · 선행 `OPS-98`) |
| 코치 대기 경험 설계(검증 후 부분 노출·진행 표시) | R3 교수학 축 | L4·mobile | 중 | `pedagogy-designer` | ✅ `PED-39` (P2) |
| 클라이언트 값싼 개선 4종(미사용 왕복·토큰 캐시·입력창·sizeOf) | R4 | mobile | 소 | 없음 | ✅ `MOB-24` (P1) |
| next-problem 조회·처치 기록 분리 | R4 prefetch 전제 | backend·L2 | 중 | 추천 효과 분석 계약 검토 필요 | ✅ `REC-12` (P2 · 선행 `OPS-95`, prefetch 본체 포함) |
| 클라이언트 흐름 구간 마커(F1~F4, 외부 전송 없음) | R5 1단계 | mobile | 소 | 없음 | ✅ `MOB-25` (P2) |
| SymPy 이벤트 루프 격리 + CPU 상한(타임아웃=판정 불가) | R6 | backend·L3 | 중 | 검증 권위 계약. 등재 시 ONNX(OCR)는 `ocr_enabled` 기본 OFF라 범위 밖으로 좁혔다 | ✅ `OPS-96` (P1) |

**12/31 내부 완성 기간에 권하는 최소 묶음**: R1 + R4(prefetch 제외) + R6. 셋 다 작고, 실사용자가 없어도 효과를 판정할 수 있다. R6은 성능이 아니라 가용성 결함이다. → 2026-09-25 Kiki 지정으로 이 3건을 등재했다. 등급은 셋 다 P1이다(12월 검증은 없어도 성립하지만 품질을 크게 높인다). 같은 날 나머지 6건도 등재했다 — 코치 턴 축(`OPS-98`·`PED-38`)만 P1이고 나머지 4건은 P2다(12월 검증과 무관한 UX·회귀 방지 축).

**선행 조건 정정 (등재 시점)**: 위 표의 R3 ①③④ 행은 원래 `OPS-75`(재측정) 선행이었다. 등재 때는 `SEC-34`(Phaiakes9 감염 대응 완결)로 바꿨다. 이 측정에 필요한 것은 '깨끗한 머신'이지 감염 기간 벤치마크 전면 재측정(`OPS-75`, P2)이 아니며, P1 태스크를 P2에 묶으면 우선순위가 뒤집히기 때문이다.
