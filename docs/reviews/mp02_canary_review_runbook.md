...
반려 = 30`으로 끝나며, §5의 이벤트 건수가 30이면 성공입니다. 실패 신호와 대처는 각 절에
함께 적었습니다.
...
**요약 JSON에서 반드시 눈으로 보실 네 줄**:

| 필드 | 기대값 | 다르면 |
|---|---|---|
| `canary_size` | 30 | 대장 기록값입니다. 다르면 회차가 다른 설정으로 돌았습니다 |
| `emitted` | `canary_size`와 같음 | 적으면 검수 분모가 줄어든 것입니다 — 아래 shortfall 참조 |
| `unresolved_count` | 0 | 0이 아니면 `unresolved` 배열에 건별 사유가 있습니다 |
| `duplicate_slug_rows` | 0 | 0이 아니면 같은 문항이 두 번 실려 실검수 건수가 산출 건수보다 적어집니다 |
...
"SLICE_WRITTEN=$($S.written)"; "EMITTED=$($S.emitted)"; "QUEUE_LINES=$QueueLines"
...
`emitted`와 같으면 완주입니다. 합이 적으면 보류·중단이 남은 것이니 `--resume`으로 이어가십시오.
...
**판정**: `VERDICT_LINES`가 §2의 `emitted`와 같으면 100% 검수입니다. `EVENT_LINES`는 타이머
...
§1의 `CANARY_SLICE_FILE` 한 줄 · §2의 `SLICE_EXIT`와 요약 JSON 전체 · §3의 네 줄 · §4의 세션 요약
전체 · §5의 세 줄과 `hit_cu_metrics` 출력. **출력을 자르지 말고 그대로** 주십시오 — 값이 잘린 채
판단하면 분모가 조용히 줄어든 것을 놓칩니다.
