...
| # | 안전장치 | 막는 반례 | 구현 |
|---|---|---|---|
| ① | **이번 회차에 새로 확인된 가설만** — 활성 가설 중 `turns_since_evidence = 0`이고 **직전 다른 응답의 마지막 원장 전이 이후에 증거로 갱신된** 것이 있어야 한다(EOS-140 정정 — 아래) | 옛 가설로 엉뚱한 개념에 고정 (비판 S1) | `misconception_hypothesis` 1행 조회 + 원장 경계 스칼라 서브쿼리(조회 수 불변) |
...
**정정(2026-09-25 · `EOS-140`) — 초판 ①은 반례 S1을 막지 못했다.** `turns_since_evidence = 0`은 "가장 최근 *스캔* 턴에서 매치됐다"이지 "이 응답에서 매치됐다"가 아니다. 응답 제출 경로의 스캔은 오답 + 답안(또는 선지 인덱스) + 문항 지문이 있어야 돌고(`api/me.py::_scan_attempt_misconceptions`), 돌지 않은 회차는 가설을 건드리지 않는다. 실 PG 재현: C 오개념 오답(가설 신뢰 0.90 · tse 0) → C 정답(미스캔 · 가설 불변) → P **답안 없는** 오답(미스캔 · 가설 불변 · 학생 전체 활성 가설로 R3 재발화) → 추천이 `applied` · 교정 대상 P · `reason.confidence` 0.9. 교과서·평가원 출처 문항은 저작권 레일상 지문이 비어 있어, 선지 채널이 없으면 오답이 구조적으로 미스캔이다 — 드문 경로가 아니다.

- **경계**: 결정 응답이 *아닌* 응답이 원장에 남긴 가장 늦은 전이 시각보다 늦게 갱신된 가설만 "이번 회차"로 인정한다. 응답 제출 경로는 스캔(가설 갱신·commit) **뒤에** 전이를 적재하므로, 그 경계 이후의 갱신은 이번 응답의 스캔(또는 두 응답 사이의 코치 대화 턴)에서 온 것이다. 두 시각 모두 DB `now()`다 — `problem_attempt`의 수신 시각은 앱 시계라 쓰지 않는다.
- **검증**: 실 PG 통합 3건 추가(반례 → `weak_misconception_evidence` · 스캔된 대조군 · 앞선 응답 뒤 새 오개념 오답 → 여전히 `applied`). 뮤테이션 6종 중 5종 RED(경계 제거 · `max`→`min` · 결정 응답 자신 포함 · 첫 응답 분기 제거 · 호출부 미전달). `>`→`>=`는 동등 뮤턴트다 — 서로 다른 요청의 트랜잭션 시각이 같아지는 현실적 입력이 없다.
- **남는 한계**: 두 응답 사이의 코치 대화 턴에서 매치된 가설도 "이번 회차"로 센다. 근본 해소는 R3의 입력을 이번 응답의 스캔 결과로 좁히는 것이며 `EOS-138`이 소유한다 — 그 태스크의 처분안도 같은 "tse 0 = 이번 회차" 전제였으므로 반례를 병기했다.
