...
>
> **⚠ 이 문서는 회차별 누적이다 — 최신 판정은 문서 *끝*에 있다.** 위 §0~§9는 **1회차**(판정 기준 `e91a75d3`) 것이고, 아래로 갈수록 최신이다: 2회차 §10~§15(`266c3d55`) → 3회차 §18~§21(`5afc1c34`) → 4회차 §22(`1671b9d4`) → **사후 처분 §23(`42a77272`)**. 각 회차의 상단 해시는 그 회차 시점의 것이며 소급 갱신하지 않는다 — 판정은 시점에 종속되므로 해시 없는 판정은 재현 불가다.
> **사후 처분 요약(§23)**: 회차 02 `P3-00b`의 소유 태스크가 `P3-00` → Phase 2 Gate 2 3차 재판정 `EOS-141`로 바뀌었다(Kiki 결정 2026-09-25 · `P3-00` 취소 · 처음엔 `EOS-130`이었으나 2차 판정 완료로 이동). §18-1 회차표 02행에 반영했다.
...
| 02 | `P3-00b` | ~~`P3-00-phase2-acceptance-check`~~ → **`EOS-141-phase2-gate2-third-rejudgment`** | 신규 → **취소·재판정 `EOS-141`에 흡수**(§23 · 2026-09-25) |
...
## §22-8. 부수 결과 처분 — Kiki 결정 2026-09-24: "기간은 줄이고, 내용은 채운다"

§22-1은 Phase 3 실효 기간이 10/26~11/15로 줄어 **Week 4 구현이 11/14까지 끝나야 한다**는 부수 결과를 적고, 대안(Release 판정을 11/22로 되돌리기)의 선택을 Kiki께 넘겼다. Kiki 결정(2026-09-24, 원문): **"기간은 단축하는 것이 이로운 것이고, 내용은 채우는 것이 유리한 것이다."**

해석과 집행 범위:

| 축 | 결정 | 이 저장소에서 뜻하는 것 |
|---|---|---|
| 기간 | **단축 유지** | 통합 게이트 `G-p3-w3-release-merged`(11/15)를 그대로 둔다. Release 판정을 11/22로 되돌리는 대안은 **기각**됐다 |
| 내용 | **채운다(축소 금지)** | 기간 압축을 이유로 Week 4 작업(`EOS-50`=P3-11 · `P3-12` · `P3-13` · `P3-14`)이나 Phase 3 Release Gate A~E 요건을 줄이지 않는다. 11/15에 요건이 미충족이면 그 판정은 **범위를 줄인 PASS가 아니라 FAIL**이다 |

**대장 변경 0건인 이유**: 두 축 모두 *현 상태를 유지하라*는 결정이다. 통합 게이트 제목이 이미 "Phase 3 Release Gate A~E 전부 충족"을 요구하고, 게이트 날짜도 이미 11/15다. 바꿀 것이 없으므로 새 게이트·amend를 만들지 않았다(만들면 같은 요건이 두 곳에 생긴다).

**남는 실행 과제(판정이 아니라 일정 운영 — 이 문서는 기록만 한다)**: 기간을 줄이고 내용을 유지하려면 Week 4 작업이 일부 **앞당겨져야** 한다. 대장상 선행 관계로 보면 `P3-12`는 `P3-10`에 걸려 있지 않고(선행 = `ADMIN-04`·`ADMIN-06`·`ADMIN-07`) Week 4 경계와 무관하게 먼저 착수할 수 있다. 반면 `EOS-50`은 `P3-10`을, `P3-13`은 `P3-12`를, `P3-14`는 `P3-13`을 선행으로 둔 직렬 사슬이다. 이 사슬을 어떻게 앞당길지는 착수 시점에 `backlog.py next`가 계산하며, 이 문서가 순서를 새로 정하지 않는다. 또한 `EOS-50`은 `eos_priority: P0`인데 `priority: 3`이라 착수 후보 순위에서 뒤로 밀릴 수 있다 — 조정이 필요하면 `amend --priority`로 하되, 이 회차에서는 하지 않았다(순위 재배정은 결정 범위 밖).

## §22-9. 기본방침 변경 — "기간은 목표이지 진도 제한이 아니다" (Kiki 지정 2026-09-24)

§22-8 결정 직후 Kiki가 이를 프로젝트 기본방침으로 올렸다(원문: "기간을 세팅한 것은 적절한 목표를 설정한 것이지 진도를 제한하기 위한 것이 아냐"). 정본 = `docs/standards/build_harness.md` §3a. 이 대조표에 미치는 영향:

| 대상 | 종전 | 변경 |
|---|---|---|
| §19 설계 원칙 "주차 경계는 게이트로, 주차 내부는 선행 체인으로 잇는다" | 주차 경계를 착수 잠금으로 집행 | **주차 판정 게이트는 유지하되 날짜가 아니라 판정 조건으로 풀린다**(조건 충족 시 조기 `clear` 허용). 주차 경계를 흉내 내는 `depends_on`은 걸지 않는다 |
| `EOS-50`(=P3-11) → `P3-10` 부착 | §22-2가 "Week 4 착수 경계는 `depends_on`이 이미 지킨다"로 둔 주차 경계 대용 | **제거**(`amend --remove-depends`). 내용 선행 `EOS-49`는 유지. 결과: `EOS-50`이 즉시 착수 후보가 됐다 |
| `P3-04` → `G-p3-g1-week1`, `P3-08` → `G-p3-g2-week2` | 게이트 제목 날짜(11/1·11/8)까지 대기로 읽힘 | **부착 유지** — 주차 판정은 앞 주차 산출물의 *품질* 확인이라 내용 축이다. 제목의 날짜는 판정 목표일로 읽는다 |
| `G-p3-w3-release-merged`(11/15) | 판정일 | 판정 **시한**. 요건이 먼저 채워지면 먼저 판정한다 |
| Not Now 12종 "11월 착수 금지" | 날짜 표현 | 실질은 **스코프**(v1.0 이후) — 방침 변경 대상이 아니다. 해제는 `G-not-now-v1-release-recheck` 판정 |

**한계**: 게이트 제목의 날짜 문자열은 고치지 않았다(게이트 정정 CLI 부재 · `HARN-124` 소유). §22-2의 "게이트가 하던 일을 의존 체인이 한다"는 이 절로 대체된다 — 이력 보존을 위해 §22-2 본문은 소급 수정하지 않는다.
...
## §23. 사후 처분 — 회차 02 `P3-00b`를 Phase 2 Gate 2 재판정에 흡수 (Kiki 결정 2026-09-25)

> **판정 기준: main `a60a1a9a`** (#1323 머지 — `EOS-130` 2차 재판정 FAIL · 후속 `EOS-141` 등재 / #1327 머지 — `EOS-139` · `EOS-26` 등재) · `EOS-128` 종결(§22-6) 뒤의 처분이라 `EOS-128`의 회차가 아니다. 소유 태스크가 없는 대장 정정 5건이다.
> **처분 대상의 이동**: 이 처분은 처음(main `42a77272`)에 `EOS-130`을 흡수처로 삼았다. 그 PR(#1321)이 머지되기 전에 다른 세션이 `EOS-130` 2차 재판정을 마쳐(#1323) `EOS-130`이 done이 됐으므로, 진입 게이트를 열 수 있는 판정인 **3차 재판정 `EOS-141`**로 옮겼다. `EOS-130`에는 이 처분의 흔적이 남지 않는다(미머지 변경을 되돌리고 `EOS-141`에 다시 적용).

**계기**: 세션이 `P3-00`을 착수하려 했으나 착수 후보 전수(`backlog.py next --n 600 --json` · 192건)에 없었다. 막은 것은 `requires_gates`의 `G-p3-entry-gate2-pass`다(pending · 당시 최신 판정 2026-09-24 FAIL).

### 23-1. 실측 — 두 태스크가 같은 일을 한다

| 요구 | 지시문 [02] `P3-00b` 원문 | `P3-00` (대장) | 재판정 태스크 (`EOS-130` → `EOS-141`) |
|---|---|---|---|
| 시작 조건 | `P3-00a` 완료 **뿐** | + 진입 게이트 `G-p3-entry-gate2-pass` | 게이트 미부착 |
| 10조건·무개입 3루프 main 재실측 | 1번 | ① | 있음 |
| KPI 5종 CLI 재실측 · 측정 불가는 "측정 실패" | 2번 | ① | 있음 |
| SCENARIO-001~010이 CI에서 실제로 돌았는가(skipped 아님) | 3번 | **없음** | **없음** (`EOS-141` ③은 로컬 실행만) |
| 미충족마다 끊긴 지점 + Phase 3 어느 항목 전에 고칠지 + 선행 등재 | 4번 | ② (`--depends` 명시) | 끊긴 지점만 (P3 태스크 지목·`--depends` 없음) |
| 3루프 축 소유 태스크 지목 | — | ③ | 없음 |

지시문 원문은 `P3-00b`를 **FAIL 상태에서 도는 작업**으로 썼다 — 시작 조건에 Gate 2 PASS가 없고, 4번이 미충족 항목의 존재를 전제한다. 대장이 진입 게이트를 부착해 순환이 생겼고(사고 대장 `gate-resolution-path-unlinked` 1회차), 그 순환을 풀려고 등재한 재판정 태스크가 요구를 겹쳐 가졌다. 이 상태로 두면 게이트가 열린 뒤 `P3-00`이 방금 PASS로 판정한 것을 한 번 더 잰다. 그리고 지시문 3번은 **어느 쪽에도 옮겨지지 않은 채** 빠져 있었다.

### 23-2. Kiki 결정과 대장 변경

세 선택지(흡수 · 그대로 대기 · 게이트를 떼고 지금 실행) 중 **흡수**. 변경은 전부 CLI 경유다.

| 종류 | ID | 내용 |
|---|---|---|
| amend | `EOS-141-phase2-gate2-third-rejudgment` | acceptance ⑧~⑪ 추가 — ⑧ SCENARIO-001~010 CI 실제 실행 확인 ⑨ 미충족마다 P3 태스크 full-id 지목 + 그 태스크에 `amend --depends` (PASS여도 KPI 공백은 이 절차를 밟는다) ⑩ 3루프 축 소유 태스크 지목 ⑪ 지시문 [02] 완료 판정 문구 승계. 제목에 "인수 점검(P3-00b 흡수)" |
| amend | `P3-01-scope-freeze` | `depends_on` −`P3-00` +`EOS-141`. 취소된 태스크를 선행으로 남기면 `deps_cancelled`로 영구 제외된다(`selector.cancelled_dependencies`) |
| cancel | `P3-00-phase2-acceptance-check` | 흡수로 취소 |
| gates amend | `G-p3-entry-gate2-pass` | 해소 태스크 `EOS-130` → `EOS-141`. done인 `EOS-130`을 입력으로 남기면 그래프가 "입력 전부 완료 · 결정 대기"로 읽어, 실제로는 다음 판정을 기다리는 상태를 가린다 |
| amend | `EOS-141-phase2-gate2-third-rejudgment` | `depends_on` +`EOS-26` — 교차 PR 공백 해소(§23-3). #1327 착지 후 부착 |

§18-1 회차표 02행도 `EOS-141`로 고쳤다 — Kiki가 회차 세션에 붙여넣을 착수 ID라, 취소된 ID가 남으면 그 세션이 첫 행위에서 막힌다.

### 23-3. 교차 PR 공백 — `EOS-141`의 선행에 `EOS-26`이 없다

#1327(`EOS-139`)의 방해 개념 프로브가 Kiki 기준 ⓐ(원인 미상 오답 직후 진단이 선수 개념으로 간다)가 현재 코드에서 **미충족**임을 보였고(`DIAGNOSIS_PROBE :: 선수지향=FAIL`), 그 수정 소유를 `EOS-26`으로 등재했다. 그런데 `EOS-141`(#1323 등재)의 선행은 `EOS-139` 하나뿐이고, `EOS-26` 쪽에도 `EOS-141` 언급이 없다. 두 태스크가 서로 다른 PR에 등재돼 어느 세션도 연결할 수 없었다. 이대로면 `EOS-139` 착지 직후 `EOS-141`이 착수 후보로 풀려 고쳐지지 않은 상태로 3차 재판정이 돌고 FAIL을 반복한다 — `EOS-130` 선행 보강(#1314)과 같은 형태다. #1327이 main에 착지한 뒤(`a60a1a9a`) `EOS-141 --depends EOS-26`을 부착했다(`amend` · 대장 정정 5번째). 이제 `EOS-141`은 `EOS-139`와 `EOS-26`이 둘 다 done이어야 착수 후보가 된다.

### 23-4. 하지 않은 것 (명시)

1. `EOS-141`의 **판정 전용 성격**(② production code 수정 금지)은 바꾸지 않았다.
2. 지시문 [02] 5번의 "인수 점검 보고서(docs/strategy/)"를 별도 파일로 요구하지 않았다 — `EOS-141` 판정문 1건이 겸한다(⑧~⑪ 머리말). 판정문 위치는 선례(9/19·9/24·9/25)대로 `docs/reviews/`다.
3. 게이트 `G-p3-entry-gate2-pass`는 **clear하지 않았다** — 해소 태스크만 재지정했다. clear는 Kiki 소유다.
4. `EOS-26`의 등급(`eos_priority: P1` · `priority: 3`)은 바꾸지 않았다. Gate 2 PASS를 막는 유일한 기능 결함이지만 등급 조정은 판단 사항이다.

---

**작성**: 2026-09-26 · 사후 처분(소유 태스크 없음 · Kiki 결정 집행) · 세션 `claude/epic-brown-8dl2nw` · 판정 기준 main `a60a1a9a`
