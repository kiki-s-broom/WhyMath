...
> **사후 처분 요약(§23)**: 회차 02 `P3-00b`의 소유 태스크가 `P3-00` → `EOS-130`으로 바뀌었다(Kiki 결정 2026-09-25 · `P3-00` 취소). §18-1 회차표 02행에 반영했다.
...
| 02 | `P3-00b` | ~~`P3-00-phase2-acceptance-check`~~ → **`EOS-130-phase2-gate2-rejudgment`** | 신규 → **취소·`EOS-130`에 흡수**(§23 · 2026-09-25) |
...
## §23. 사후 처분 — 회차 02 `P3-00b`를 `EOS-130`에 흡수 (Kiki 결정 2026-09-25)

> **판정 기준: main `42a77272`** · `EOS-128` 종결(§22-6) 뒤의 처분이라 `EOS-128`의 회차가 아니다. 소유 태스크가 없는 대장 정정 3건이다.

**계기**: 세션이 `P3-00`을 착수하려 했으나 착수 후보 전수(`backlog.py next --n 600 --json` · 192건)에 없었다. 막은 것은 `requires_gates`의 `G-p3-entry-gate2-pass`다(pending · 최신 판정 2026-09-24 FAIL).

### 23-1. 실측 — 두 태스크가 같은 일을 한다

| 요구 | 지시문 [02] `P3-00b` 원문 | `P3-00` (대장) | `EOS-130` (대장 · 이번 정정 전) |
|---|---|---|---|
| 시작 조건 | `P3-00a` 완료 **뿐** | + 진입 게이트 `G-p3-entry-gate2-pass` | 게이트 미부착 · 선행 `EOS-21`·`EOS-24`·`EOS-124` |
| 10조건·무개입 3루프 main 재실측 | 1번 | ① | ① ④ |
| KPI 5종 CLI 재실측 · 측정 불가는 "측정 실패" | 2번 | ① | ⑤ |
| SCENARIO-001~010이 CI에서 실제로 돌았는가(skipped 아님) | 3번 | **없음** | **없음** |
| 미충족마다 끊긴 지점 + Phase 3 어느 항목 전에 고칠지 + 선행 등재 | 4번 | ② (`--depends` 명시) | ⑥ ⑧ (P3 태스크 지목·`--depends` 없음) |
| 3루프 축 소유 태스크 지목 | — | ③ | ④ (상태만) |

지시문 원문은 `P3-00b`를 **FAIL 상태에서 도는 작업**으로 썼다 — 시작 조건에 Gate 2 PASS가 없고, 4번이 미충족 항목의 존재를 전제한다. 대장이 진입 게이트를 부착해 순환이 생겼고(미머지 PR #1320이 사고 대장 `gate-resolution-path-unlinked` 1회차로 기록 중), 그 순환을 풀려고 등재한 `EOS-130`(§22-3)이 요구를 그대로 겹쳐 가졌다. 이 상태로 두면 게이트가 열린 뒤 `P3-00`이 방금 PASS로 판정한 것을 한 번 더 잰다. 그리고 지시문 3번은 **어느 쪽에도 옮겨지지 않은 채** 빠져 있었다.
...
| 종류 | ID | 내용 |
|---|---|---|
| amend | `EOS-130-phase2-gate2-rejudgment` | acceptance ⑨~⑫ 추가 — ⑨ SCENARIO-001~010 실제 실행 확인 ⑩ 미충족마다 P3 태스크 full-id 지목 + 그 태스크에 `amend --depends` (PASS여도 KPI 공백은 이 절차를 밟는다) ⑪ 3루프 축 소유 태스크 지목 ⑫ 지시문 [02] 완료 판정 문구 승계. 제목에 "인수 점검(P3-00b 흡수)" |
| amend | `P3-01-scope-freeze` | `depends_on` −`P3-00` +`EOS-130`. 취소된 태스크를 선행으로 남기면 `deps_cancelled`로 영구 제외된다(`selector.cancelled_dependencies`) |
| cancel | `P3-00-phase2-acceptance-check` | 흡수로 취소 |

§18-1 회차표 02행도 `EOS-130`으로 고쳤다 — Kiki가 회차 세션에 붙여넣을 착수 ID라, 취소된 ID가 남으면 그 세션이 첫 행위에서 막힌다.

검증: `backlog.py validate` green(태스크 856 · 게이트 83 · 트랙 3) · `audit-deps` 위반 0건 · 쓴 뒤 대장 YAML을 다시 파싱해 ⑨~⑫의 핵심 식별자 누락 0건.

### 23-3. 하지 않은 것 (명시)

1. `EOS-130`의 **판정 전용 성격**(② production code 수정 금지)과 **선행**(`EOS-21`·`EOS-24`·`EOS-124`)은 바꾸지 않았다. `EOS-24`(PR #1316)·`EOS-124`(PR #1317)는 2026-09-25 기준 CI green·미머지이며, 머지는 Kiki 판단으로 남겼다.
2. 지시문 [02] 5번의 "인수 점검 보고서(docs/strategy/)"를 별도 파일로 요구하지 않았다 — `EOS-130` 판정문 1건이 겸한다(⑨~⑫ 머리말). 판정문 위치는 선례(9/19·9/24)대로 `docs/reviews/`다.
3. 게이트 `G-p3-entry-gate2-pass`는 건드리지 않았다 — Kiki 소유이고 이 처분과 무관하게 pending이다.
4. PR #1316 본문에 따르면 `EOS-24`가 머지돼도 Loop 1은 절반만 풀린다(일반 오답은 새 결정 게이트 `G-eos24-loop1-undiagnosed-wrong-criterion`으로 이관 · 미머지). 따라서 두 PR 머지만으로 재판정 PASS가 보장되지 않는다 — 이 절은 그 판정을 하지 않는다.

---

**작성**: 2026-09-25 · 사후 처분(소유 태스크 없음 · Kiki 결정 집행) · 세션 `claude/epic-brown-8dl2nw` · 판정 기준 main `42a77272`
