...
- ~~I3 | 깊이앵커 학생 표본 20~30명 반응 수집 | P4 | T2 | 10 | Spearman ρ 산출 가능 데이터~~ → **2027 이월**(2026-09-22 · G4 각주 참조). T2라 실행계획 354h에 미포함이었으므로 예산 무변동.
...
| G4 계측기 완성 | 12-13(일) | 깊이앵커 수동 개입 0 루프 3연속 + P0 결함 0 ~~+ 학생 표본 ≥20명~~[^g4-sample] |
| G5 동결→판정 | 12-27→12-31 | (판정 게이트) Go/Conditional/No-Go + 2027 범위 결정 |

[^g4-sample]: **2026-09-22 Kiki 결정 — G4에서 "학생 표본 ≥20명"을 제외하고 12월 판정을 HIT(콘텐츠 생산성) 축으로 한정한다.** 초판 조건은 이 문서 **§0-2와 정면으로 모순**이었다 — §0-2는 "베타 사용자 확보는 전부 2027년 범위"라고 정해 두고서 부록 E는 12-13에 학생 표본 20명을 요구했다. 8/30 선언 이래 그 모순을 아무도 판정한 적이 없고, 2026-09-22 지시("12/31까지 내부적으로 앱을 완성하고 그 이후로 테스트 과정을 미룬다")가 §0-2 쪽으로 확정했다. **정합성 확인(실측)**: 실패정의 F-Ⅰ~F-Ⅴ 어느 것도 학생 표본을 요구하지 않는다(F-Ⅰ HIT · F-Ⅱ 오류율 · F-Ⅲ 실패 분포 · F-Ⅳ 앵커 미달 수 · F-Ⅴ 누설률) — 따라서 이 제외는 G0(9/6)에 동결된 실패정의를 건드리지 않으며, 동결 대상은 검증설계서 §5이고 이 행은 거기에 없다. 부록 D의 **I3(깊이앵커 학생 표본 20~30명 반응 수집)도 함께 2027로 이월**된다 — I3는 애초에 T2(선택)라 실행계획 354h(≈T1 352h)에 들어 있지 않았으므로 시간 예산은 변하지 않는다. 재개·재판정 지점 = 게이트 `G-pilot-defer-2027-recheck`(2027-01 첫 계획 세션). 이 결정의 회귀는 `tests/backend/schema/test_declaration_canon_consistency.py`가 기계로 동결한다.
...
- **2026-09-22 G4 학생 표본 조건 제외** (Kiki 결정 · 선택지 ⓐ): 부록 E G4에서 "학생 표본 ≥20명"을 빼고 12월 판정을 **HIT(콘텐츠 생산성) 축으로 한정**한다 · 부록 D I3(학생 표본 반응 수집·T2)를 2027로 이월 · 파일럿(`S3-01`)은 게이트 `G-pilot-defer-2027-recheck`로 연말 이후 연기. 제외 이유는 G4 행 각주(**§0-2와 부록 E의 모순**을 §0-2 쪽으로 확정)에 있다. 초판 값은 취소선으로 보존했다(2026-08-31 선례). 실패정의 F-Ⅰ~F-Ⅴ·검증설계서 §5 해시 동결은 **무변경**이다.
