...
| G4 계측기 완성 | 12-13(일) | 깊이앵커 수동 개입 0 루프 3연속 + **내부 테스트 계정 표본 ≥20건**(실사용자 아님을 판정문에 명시) + P0 결함 0 — 2026-09-24 Kiki 결정(EOS-135): 원 조건 '학생 표본 ≥20명'은 학생 참여 과정 연기(ARCH-66 ⑥)로 12-13 충족 불가라 **조건을 교체하고 판정일은 유지**한다. 실제 학생 표본 ≥20명은 12/31 이후 `G-student-work-after-internal-completion`에서 판정. 테스트 계정 표본을 실사용자 검증으로 계상하지 않는다 |
...
- **2026-09-24 G4 학생 표본 조건 교체** (Kiki 결정 · `EOS-135` — PR #1304): 부록 E G4의 차단 조건 중 "학생 표본 ≥20명"을 **"내부 테스트 계정 표본 ≥20건(실사용자 아님을 판정문에 명시)"**으로 교체하고 **판정일 12-13은 유지**한다. 실제 학생 표본 ≥20명은 12/31 이후 게이트 `G-student-work-after-internal-completion`에서 판정하며, 테스트 계정 표본을 실사용자 검증으로 계상하지 않는다. 배경 = 학생 참여 과정 연기(`ARCH-66` ⑥)로 원 조건이 12-13 충족 불가가 된 것. 실패정의 F-Ⅰ~F-Ⅴ·검증설계서 §5 해시 동결은 **무변경**이다.
  이 행의 조건은 이제 기계로 동결된다 — `TestG4ConditionFreeze`(`EOS-143`). 등재 이유: 교체 당시 **부록 E를 읽는 기계 장치가 0건**이라, 8/30 선언 이래 §0-2("베타 사용자 확보는 전부 2027년 범위")와 G4가 서로 모순인 채로 3주 넘게 아무도 보지 못했다 — A7과 같은 형태의 재발이다.
