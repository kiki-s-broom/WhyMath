...
| `P-15` 페르소나 3종 안정화 | §11 | ~~amend~~ → **신규 등재**(2026-09-19 정정 — 아래 §11.6) | `EOS-122` |
| `P-16` Gate 2 최종 판정(10/25) | §18 | **등재 제외** — 빌드 항목이 아니라 게이트 판정. 게다가 §9-① 일정 충돌 미해소 | §9-① |

**16종 전건에 처분이 대응한다** — 신규 등재 **9건** · amend 1건 · 기존충족 3건 · 등재 제외 **2건** · (P-11은 P-03과 같은 태스크로 수렴). *`P-01`은 2026-09-16 정정으로 등재 제외 → 신규 등재(§11.4).*
...
### §11.6. `P-15` 처분 정정 (2026-09-19 · `EOS-122`)

위 §11.2 표는 `P-15`(페르소나 3종 안정화)의 처분을 `PED-36` **amend**로 지정했고,
그 amend는 2026-09-16에 `PED-36` acceptance ⑪로 실제 착지했다. **그 판정은 그 시점에 옳았다.**

정정 사유는 판정이 틀려서가 아니라 **착수가 불가능해졌기 때문**이다. 2026-09-19 `P-15` 집행
세션이 지정대로 `PED-36`을 claim하려 했을 때, 25초 전(`2026-09-19T14:07:55Z`)에 타 세션이
먼저 원격 claim해 `backlog.py start`가 exit 1로 거부했다. 그 브랜치는 미push 상태라 작업 범위를
읽을 수도 없었다. 착수 순서 규칙(CLAUDE.md 2026-09-06 — claim 전 코드 읽기·쓰기 금지)상
`PED-36` 위에서 진행할 수 없었다.

Kiki가 `EOS-120` ③의 해소 3택 중 **(다) 축 분리**를 판정해, ⑪만 떼어 `EOS-122`로 신규 등재했다.
`PED-36` 쪽에는 ⑪을 완료 조건으로 대조하지 말라는 **폐기 표시(tombstone)**를 amend로 남겼다
(append 전용이라 원문 삭제 불가 — `HARN-76` ④).

분리 후 경계:

| 원 `PED-36` 축 | 소유자 |
|---|---|
| ⑧ SCENARIO-001~010 정형화 · ⑩ CI 상시 | `EOS-119-phase2-scenario-regression-suite` |
| ⑪ 페르소나 3인 완주 | `EOS-122-persona-three-journey-completion` |
| ①②③ 시나리오 뱅크 스키마·시험지 소스 배선 · ⑨ 코드갭/테스트갭 분류 · ⑫ 연속 3루프 | `PED-36` 잔존 |

이로써 `EOS-120`이 등재한 중복 소유 중 **`P-15` 축은 해소됐다**. `P-13` 축(⑧⑩ ↔ `EOS-119`)의
폐기 표시와 `EOS-120` 자신의 종결은 이 정정의 범위 밖이며 `EOS-120`이 계속 소유한다.
