...
SEC-09(`audit.py`)는 개인정보·콘텐츠 감사 6종(반출·동의변경·관리자접근·역할변경·콘텐츠CUD·
운영자토큰발급) writer + IP 해싱을 더한다(역할변경은 ADMIN-01·콘텐츠CUD는 SEC-29·운영자토큰발급은
ADMIN-15가 추가 — 각각 `ops/role_grant_cli.py`·`api/concepts.py`/`api/problems.py`·
`ops/operator_token_cli.py`가 생산자).
"""
